			    ...
			    const struct i2c_device_id *id)
{
	...
	es8316->spk_mute_gpio = of_get_named_gpio_flags(np,
						       ...
						       0,
						       &flags);
	if (es8316->spk_mute_gpio < 0) {
		dev_info(&i2c->dev, "Can not read property spk_mute_gpio\n");
		es8316->spk_mute_gpio = INVALID_GPIO;
	} else {
		es8316->spk_active_level = !(flags & OF_GPIO_ACTIVE_LOW);
		ret = devm_gpio_request_one(&i2c->dev, es8316->spk_mute_gpio,
					    GPIOF_DIR_OUT, NULL);
		if (ret) {
